...
class Tap : public InputDevice, public OutputDevice
{
    public:
        ...
        int Read(vector<char> &buf)
        {
            ...
            packet_ethernet_arp_ipv4_t* packet_ethernet_arp_ipv4 =
                (packet_ethernet_arp_ipv4_t*)&buf[FRAME_LENGTH_IN_BYTES];
            ...
            int is_ip_ok = (0 == memcmp(&(arp->target_ip), TAP1_IP_ADDR,
                                        sizeof(TAP1_IP_ADDR)));
            if (!is_ip_ok)
            {
                return -1;
        ...
        int getMac(const char* name, uint8_t* mac)
        {
            ...
            Debug_LOG_DEBUG("MAC %02x:%02x:%02x:%02x:%02x:%02x", mac[0], mac[1], mac[2],
                            mac[3], mac[4], mac[5]);
        ...
        {
            UartSocketGuestSocketCommand command =
                static_cast<UartSocketGuestSocketCommand>(buffer[0]);
